...
const YULIETH_PREFIX = '/api/v1/yulieth-galarza';

const rawRegistry: ServiceConfig[] = [
  {
    name: 'auth-service',
...
export const registry: ServiceConfig[] = rawRegistry.map(service => {
  const extendedPrefixes = [...service.pathPrefixes];
  for (const prefix of service.pathPrefixes) {
    if (prefix.startsWith('/api/v1/')) {
      const subPath = prefix.replace('/api/v1/', '');
      extendedPrefixes.push(`${YULIETH_PREFIX}/${subPath}`);
    }
  }
  return {
    ...service,
    pathPrefixes: extendedPrefixes,
  };
});
